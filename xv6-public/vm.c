...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  a = PGROUNDUP(oldsz); //할당 페이지 시작 주소: oldsize을 반올림
  for(; a < newsz; a += PGSIZE){ 
    ...
    memset(mem, 0, PGSIZE);
    if(mappages(pgdir, (char*)a, PGSIZE, V2P(mem), PTE_W|PTE_U) < 0){ // pagetable에 매핑
...
lazy_allocation()
{
	...
	uint va = PGROUNDDOWN(pfglt_addr);

	char *mem = kalloc();
	if(mem == 0) {
	  cprintf("lazy_allocation out of memory\n");
	  return -1;
	}
	memset(mem, 0, PGSIZE);
